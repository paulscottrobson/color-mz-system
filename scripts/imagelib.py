...
class MZImage(object):
	...
	def addDictionary(self,name,page,address,isMacroEntry):
		...
		addDict = self.macroDictionary if isMacroEntry else self.wordDictionary
	...
	def removeDictionaryPrivateWords(self):
		self.wordDictionary = self.privateRemove(self.wordDictionary)
	...
	def loadDictionary(self):
		...
		while self.read(dPage,p) != 0:
			...
			target = self.macroDictionary if (sByte & 0x80) !=0 else self.wordDictionary 
			target[name] = [name,self.read(dPage,p+1),self.read(dPage,p+2)+self.read(dPage,p+3)*256,(sByte & 0x80) != 0]
	...
	def findWord(self,word,isMacro):
		source = self.macroDictionary if isMacro else self.wordDictionary
